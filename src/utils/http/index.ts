...
import { getToken, refreshToken, isRefreshRequest, cleanToken } from './auth'
...
class Http {
  ...
  private httpInterceptorsRequest(): void {
    Http.axiosInstance.interceptors.request.use(
      (config) => {
        if (config.url !== API_URL.postLogAdd) {
          // NProgress.start()
        }
  ...
  public request<T>(paramConfig: AxiosRequestConfig): Promise<T> {
    const config = {
      ...
      ...paramConfig,
      headers: {
        ...Http.axiosConfigDefault.headers,
        ...paramConfig.headers,
        source: sessionStorage.getItem('source')
      }
    }
    return new Promise(async (resolve, reject) => {
      if (!isRefreshRequest(config) && isInvalidToken) {
        const isSuccess = await refreshToken()

        if (isSuccess) {
          ;(config as any).headers['X-Access-Token'] = getToken()
        }
      }
